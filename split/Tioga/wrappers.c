...
OBJ_PTR FM_show_axis(OBJ_PTR fmkr, OBJ_PTR loc) { int ierr=0;
   return c_show_axis(fmkr, Get_FM(fmkr, &ierr), Number_to_int(loc, &ierr), &ierr); } 
OBJ_PTR FM_show_edge(OBJ_PTR fmkr, OBJ_PTR loc) { int ierr=0;
   return c_show_edge(fmkr, Get_FM(fmkr, &ierr), Number_to_int(loc, &ierr), &ierr); }
OBJ_PTR FM_no_title(OBJ_PTR fmkr) { int ierr=0; return c_no_title(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_no_xlabel(OBJ_PTR fmkr) { int ierr=0; return c_no_xlabel(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_no_ylabel(OBJ_PTR fmkr) { int ierr=0; return c_no_ylabel(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_no_xaxis(OBJ_PTR fmkr) { int ierr=0; return c_no_xaxis(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_no_yaxis(OBJ_PTR fmkr) { int ierr=0; return c_no_yaxis(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_no_left_edge(OBJ_PTR fmkr) { int ierr=0; return c_no_left_edge(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_no_right_edge(OBJ_PTR fmkr) { int ierr=0; return c_no_right_edge(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_no_top_edge(OBJ_PTR fmkr) { int ierr=0; return c_no_top_edge(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_no_bottom_edge(OBJ_PTR fmkr) { int ierr=0; return c_no_bottom_edge(fmkr, Get_FM(fmkr, &ierr), &ierr); }

// init.c
OBJ_PTR FM_private_init_fm_data(OBJ_PTR fmkr) { int ierr=0; 
   return c_private_init_fm_data(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_set_frame_sides(OBJ_PTR fmkr, OBJ_PTR left, OBJ_PTR right, OBJ_PTR top, OBJ_PTR bottom) { int ierr=0;
   return c_set_frame_sides(fmkr, Get_FM(fmkr, &ierr),
...
OBJ_PTR FM_set_device_pagesize(OBJ_PTR fmkr, OBJ_PTR width, OBJ_PTR height) { int ierr=0;
   return c_set_device_pagesize(fmkr, Get_FM(fmkr, &ierr), Number_to_double(width, &ierr), Number_to_double(height, &ierr), &ierr); }
...
OBJ_PTR FM_private_make(OBJ_PTR fmkr, OBJ_PTR name, OBJ_PTR cmd) { int ierr=0;
   return c_private_make(fmkr, Get_FM(fmkr, &ierr), name, cmd, &ierr); }
OBJ_PTR FM_private_make_portfolio(OBJ_PTR fmkr, OBJ_PTR name, OBJ_PTR fignums, OBJ_PTR fignames) { int ierr=0;
   return c_private_make_portfolio(fmkr, Get_FM(fmkr, &ierr), name, fignums, fignames, &ierr); }
...
OBJ_PTR FM_stroke_opacity_set(OBJ_PTR fmkr, OBJ_PTR val) { int ierr=0;
   c_stroke_opacity_set(fmkr, Get_FM(fmkr, &ierr), Number_to_double(val, &ierr), &ierr); return val; }
OBJ_PTR FM_fill_opacity_set(OBJ_PTR fmkr, OBJ_PTR val) { int ierr=0;
   c_fill_opacity_set(fmkr, Get_FM(fmkr, &ierr), Number_to_double(val, &ierr), &ierr); return val; }
OBJ_PTR FM_private_axial_shading(
   ...
   OBJ_PTR colormap, OBJ_PTR extend_start, OBJ_PTR extend_end) { int ierr=0;
      return c_private_axial_shading(fmkr, Get_FM(fmkr, &ierr), Number_to_double(x0, &ierr), Number_to_double(y0, &ierr),
...
OBJ_PTR FM_private_radial_shading(OBJ_PTR fmkr,
   ...
   return c_private_radial_shading(fmkr, Get_FM(fmkr, &ierr), 
...
OBJ_PTR FM_title_color_set(OBJ_PTR fmkr, OBJ_PTR val) { int ierr=0;
   return c_title_color_set(fmkr, Get_FM(fmkr, &ierr), val, &ierr); }
...
OBJ_PTR FM_xlabel_color_set(OBJ_PTR fmkr, OBJ_PTR val) { int ierr=0; 
   return c_xlabel_color_set(fmkr, Get_FM(fmkr, &ierr), val, &ierr); }
...
OBJ_PTR FM_ylabel_color_set(OBJ_PTR fmkr, OBJ_PTR val) { int ierr=0;
   return c_ylabel_color_set(fmkr, Get_FM(fmkr, &ierr), val, &ierr); }
...
OBJ_PTR FM_xaxis_stroke_color_set(OBJ_PTR fmkr, OBJ_PTR val) { int ierr=0;
   return c_xaxis_stroke_color_set(fmkr, Get_FM(fmkr, &ierr), val, &ierr); }
...
OBJ_PTR FM_yaxis_stroke_color_set(OBJ_PTR fmkr, OBJ_PTR val) { int ierr=0;
   return c_yaxis_stroke_color_set(fmkr, Get_FM(fmkr, &ierr), val, &ierr); }
...
OBJ_PTR FM_private_set_subframe(OBJ_PTR fmkr, OBJ_PTR left_margin, OBJ_PTR right_margin, 
   ...
   return c_set_subframe(fmkr, Get_FM(fmkr, &ierr), Number_to_double(left_margin, &ierr), Number_to_double(right_margin, &ierr), 
      Number_to_double(top_margin, &ierr), Number_to_double(bottom_margin, &ierr), &ierr);}
OBJ_PTR FM_private_set_default_font_size(OBJ_PTR fmkr, OBJ_PTR size) { int ierr=0;
   return c_private_set_default_font_size(fmkr, Get_FM(fmkr, &ierr), Number_to_double(size, &ierr), &ierr); }
...
OBJ_PTR FM_convert_output_to_figure_dy(OBJ_PTR fmkr, OBJ_PTR v) { int ierr=0;
   return c_convert_output_to_figure_dy(fmkr, Get_FM(fmkr, &ierr), Number_to_double(v, &ierr), &ierr); }
OBJ_PTR FM_doing_subplot(OBJ_PTR fmkr) { int ierr=0; return c_doing_subplot(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_doing_subfigure(OBJ_PTR fmkr) { int ierr=0; return c_doing_subfigure(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_private_set_bounds(OBJ_PTR fmkr, OBJ_PTR left, OBJ_PTR right, OBJ_PTR top, OBJ_PTR bottom) { int ierr=0;
   return c_private_set_bounds(fmkr, Get_FM(fmkr, &ierr), Number_to_double(left, &ierr), Number_to_double(right, &ierr),
      Number_to_double(top, &ierr), Number_to_double(bottom, &ierr), &ierr); }

// pdffile.c
OBJ_PTR FM_pdf_gsave(OBJ_PTR fmkr) { int ierr=0; return c_pdf_gsave(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_pdf_grestore(OBJ_PTR fmkr) { int ierr=0; return c_pdf_grestore(fmkr, Get_FM(fmkr, &ierr), &ierr); }
...
OBJ_PTR FM_private_show_jpg(OBJ_PTR fmkr, OBJ_PTR filename, 
   ...
   return c_private_show_jpg(fmkr, Get_FM(fmkr, &ierr), String_Ptr(filename, &ierr), 
      Number_to_int(width, &ierr), Number_to_int(height, &ierr), image_destination, Number_to_int(mask_obj_num, &ierr), &ierr); }
...
OBJ_PTR FM_stroke_color_set(OBJ_PTR fmkr, OBJ_PTR value) { int ierr=0; 
   return c_stroke_color_set(fmkr, Get_FM(fmkr, &ierr), value, &ierr); }
...
OBJ_PTR FM_fill_color_set(OBJ_PTR fmkr, OBJ_PTR value) { int ierr=0;
   return c_fill_color_set(fmkr, Get_FM(fmkr, &ierr), value, &ierr); }
...
OBJ_PTR FM_line_width_set(OBJ_PTR fmkr, OBJ_PTR val) { int ierr=0; 
   c_line_width_set(fmkr, Get_FM(fmkr, &ierr), Number_to_double(val, &ierr), &ierr); return val; }
OBJ_PTR FM_rescale_lines(OBJ_PTR fmkr, OBJ_PTR scaling_factor) { int ierr=0;
   return c_rescale_lines(fmkr, Get_FM(fmkr, &ierr), Number_to_double(scaling_factor, &ierr), &ierr); }
OBJ_PTR FM_line_cap_set(OBJ_PTR fmkr, OBJ_PTR val) { int ierr=0;
   c_line_cap_set(fmkr, Get_FM(fmkr, &ierr), Number_to_int(val, &ierr), &ierr); return val; }
OBJ_PTR FM_line_join_set(OBJ_PTR fmkr, OBJ_PTR val) { int ierr=0;
   c_line_join_set(fmkr, Get_FM(fmkr, &ierr), Number_to_int(val, &ierr), &ierr); return val; }
OBJ_PTR FM_miter_limit_set(OBJ_PTR fmkr, OBJ_PTR val) { int ierr=0;
   c_miter_limit_set(fmkr, Get_FM(fmkr, &ierr), Number_to_double(val, &ierr), &ierr); return val; }
OBJ_PTR FM_line_type_set(OBJ_PTR fmkr, OBJ_PTR line_type) { int ierr=0;
   return c_line_type_set(fmkr, Get_FM(fmkr, &ierr), line_type, &ierr); }
OBJ_PTR FM_update_bbox(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y) { int ierr=0;
   return c_update_bbox(fmkr, Get_FM(fmkr, &ierr), Number_to_double(x, &ierr), Number_to_double(y, &ierr), &ierr); }
OBJ_PTR FM_bbox_left(OBJ_PTR fmkr) { int ierr=0; return c_bbox_left(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_bbox_right(OBJ_PTR fmkr) { int ierr=0; return c_bbox_right(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_bbox_top(OBJ_PTR fmkr) { int ierr=0; return c_bbox_top(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_bbox_bottom(OBJ_PTR fmkr) { int ierr=0; return c_bbox_bottom(fmkr, Get_FM(fmkr, &ierr), &ierr); }
...
OBJ_PTR FM_append_curve_to_path(OBJ_PTR fmkr, OBJ_PTR x1, OBJ_PTR y1, OBJ_PTR x2, OBJ_PTR y2, OBJ_PTR x3, OBJ_PTR y3) { int ierr=0;
   return c_append_curve_to_path(fmkr, Get_FM(fmkr, &ierr), Number_to_double(x1, &ierr), Number_to_double(y1, &ierr), 
      Number_to_double(x2, &ierr), Number_to_double(y2, &ierr), 
      Number_to_double(x3, &ierr), Number_to_double(y3, &ierr), &ierr); }
OBJ_PTR FM_close_path(OBJ_PTR fmkr) { int ierr=0; return c_close_path(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_append_arc_to_path(OBJ_PTR fmkr, OBJ_PTR x_start, OBJ_PTR y_start, OBJ_PTR x_corner, OBJ_PTR y_corner,
   ...
   return c_append_arc_to_path(fmkr, Get_FM(fmkr, &ierr),
...
OBJ_PTR FM_append_rect_to_path(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR width, OBJ_PTR height) { int ierr=0;
   return c_append_rect_to_path(fmkr, Get_FM(fmkr, &ierr), Number_to_double(x, &ierr), Number_to_double(y, &ierr), 
...
OBJ_PTR FM_append_rounded_rect_to_path(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, 
   ...
   return c_append_rounded_rect_to_path(fmkr, Get_FM(fmkr, &ierr),
...
OBJ_PTR FM_append_oval_to_path(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR dx, OBJ_PTR dy, OBJ_PTR angle) { int ierr=0;
   return c_append_oval_to_path(fmkr, Get_FM(fmkr, &ierr),
      Number_to_double(x, &ierr), Number_to_double(y, &ierr), Number_to_double(dx, &ierr), Number_to_double(dy, &ierr), Number_to_double(angle, &ierr), &ierr); }
OBJ_PTR FM_append_circle_to_path(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR dx) { int ierr=0;
   return c_append_circle_to_path(fmkr, Get_FM(fmkr, &ierr), Number_to_double(x, &ierr), Number_to_double(y, &ierr), Number_to_double(dx, &ierr), &ierr); } 
OBJ_PTR FM_append_points_to_path(OBJ_PTR fmkr, OBJ_PTR x_vec, OBJ_PTR y_vec) { int ierr=0;
   return c_append_points_to_path(fmkr, Get_FM(fmkr, &ierr), x_vec, y_vec, &ierr); } 
OBJ_PTR FM_private_append_points_with_gaps_to_path(OBJ_PTR fmkr, OBJ_PTR x_vec, OBJ_PTR y_vec, OBJ_PTR gaps, OBJ_PTR close_gaps) { int ierr=0;
   return c_private_append_points_with_gaps_to_path(fmkr, Get_FM(fmkr, &ierr), x_vec, y_vec, gaps, (close_gaps == OBJ_TRUE), &ierr); }
OBJ_PTR FM_stroke(OBJ_PTR fmkr) { int ierr=0; return c_stroke(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_close_and_stroke(OBJ_PTR fmkr) { int ierr=0; return c_close_and_stroke(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_fill(OBJ_PTR fmkr) { int ierr=0; return c_fill(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_discard_path(OBJ_PTR fmkr) { int ierr=0; return c_discard_path(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_eofill(OBJ_PTR fmkr) { int ierr=0; return c_eofill(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_fill_and_stroke(OBJ_PTR fmkr) { int ierr=0; return c_fill_and_stroke(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_eofill_and_stroke(OBJ_PTR fmkr) { int ierr=0; return c_eofill_and_stroke(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_close_fill_and_stroke(OBJ_PTR fmkr) { int ierr=0; return c_close_fill_and_stroke(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_close_eofill_and_stroke(OBJ_PTR fmkr) { int ierr=0; return c_close_eofill_and_stroke(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_clip(OBJ_PTR fmkr) { int ierr=0; return c_clip(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_eoclip(OBJ_PTR fmkr) { int ierr=0; return c_eoclip(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_fill_and_clip(OBJ_PTR fmkr) { int ierr=0; return c_fill_and_clip(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_stroke_and_clip(OBJ_PTR fmkr) { int ierr=0; return c_stroke_and_clip(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_fill_stroke_and_clip(OBJ_PTR fmkr) { int ierr=0; return c_fill_stroke_and_clip(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_stroke_line(OBJ_PTR fmkr, OBJ_PTR x1, OBJ_PTR y1, OBJ_PTR x2, OBJ_PTR y2) { int ierr=0;
   return c_stroke_line(fmkr, Get_FM(fmkr, &ierr), 
      Number_to_double(x1, &ierr), Number_to_double(y1, &ierr), 
      Number_to_double(x2, &ierr), Number_to_double(y2, &ierr), &ierr); }
OBJ_PTR FM_fill_rect(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR width, OBJ_PTR height) { int ierr=0;
   return c_fill_rect(fmkr, Get_FM(fmkr, &ierr), 
      Number_to_double(x, &ierr), Number_to_double(y, &ierr), 
      Number_to_double(width, &ierr), Number_to_double(height, &ierr), &ierr); }
OBJ_PTR FM_stroke_rect(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR width, OBJ_PTR height) { int ierr=0;
   return c_stroke_rect(fmkr, Get_FM(fmkr, &ierr), 
      Number_to_double(x, &ierr), Number_to_double(y, &ierr), 
      Number_to_double(width, &ierr), Number_to_double(height, &ierr), &ierr); }
OBJ_PTR FM_fill_and_stroke_rect(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR width, OBJ_PTR height) { int ierr=0;
   return c_fill_and_stroke_rect(fmkr, Get_FM(fmkr, &ierr), 
      Number_to_double(x, &ierr), Number_to_double(y, &ierr), 
      Number_to_double(width, &ierr), Number_to_double(height, &ierr), &ierr); }
OBJ_PTR FM_clip_rect(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR width, OBJ_PTR height) { int ierr=0;
   return c_clip_rect(fmkr, Get_FM(fmkr, &ierr),
      Number_to_double(x, &ierr), Number_to_double(y, &ierr),
      Number_to_double(width, &ierr), Number_to_double(height, &ierr), &ierr); }
OBJ_PTR FM_clip_oval(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR dx, OBJ_PTR dy, OBJ_PTR angle) { int ierr=0;
   return c_clip_oval(fmkr, Get_FM(fmkr, &ierr), Number_to_double(x, &ierr), Number_to_double(y, &ierr), 
      Number_to_double(dx, &ierr), Number_to_double(dy, &ierr), Number_to_double(angle, &ierr), &ierr); }
OBJ_PTR FM_fill_oval(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR dx, OBJ_PTR dy, OBJ_PTR angle) { int ierr=0;
   return c_fill_oval(fmkr, Get_FM(fmkr, &ierr), Number_to_double(x, &ierr), Number_to_double(y, &ierr), 
      Number_to_double(dx, &ierr), Number_to_double(dy, &ierr), Number_to_double(angle, &ierr), &ierr); }
OBJ_PTR FM_stroke_oval(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR dx, OBJ_PTR dy, OBJ_PTR angle) { int ierr=0;
   return c_stroke_oval(fmkr, Get_FM(fmkr, &ierr), Number_to_double(x, &ierr), Number_to_double(y, &ierr), 
      Number_to_double(dx, &ierr), Number_to_double(dy, &ierr), Number_to_double(angle, &ierr), &ierr); }
OBJ_PTR FM_fill_and_stroke_oval(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR dx, OBJ_PTR dy, OBJ_PTR angle) { int ierr=0;
   return c_fill_and_stroke_oval(fmkr, Get_FM(fmkr, &ierr), Number_to_double(x, &ierr), Number_to_double(y, &ierr), 
      Number_to_double(dx, &ierr), Number_to_double(dy, &ierr), Number_to_double(angle, &ierr), &ierr); }
OBJ_PTR FM_clip_rounded_rect(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR width, OBJ_PTR height, OBJ_PTR dx, OBJ_PTR dy) { int ierr=0;
   return c_clip_rounded_rect(fmkr, Get_FM(fmkr, &ierr), Number_to_double(x, &ierr), Number_to_double(y, &ierr), 
      Number_to_double(width, &ierr), Number_to_double(height, &ierr), Number_to_double(dx, &ierr), Number_to_double(dy, &ierr), &ierr); }
OBJ_PTR FM_fill_rounded_rect(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR width, OBJ_PTR height, OBJ_PTR dx, OBJ_PTR dy) { int ierr=0;
   return c_fill_rounded_rect(fmkr, Get_FM(fmkr, &ierr), Number_to_double(x, &ierr), Number_to_double(y, &ierr), 
      Number_to_double(width, &ierr), Number_to_double(height, &ierr), Number_to_double(dx, &ierr), Number_to_double(dy, &ierr), &ierr); }
OBJ_PTR FM_stroke_rounded_rect(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR width, OBJ_PTR height, OBJ_PTR dx, OBJ_PTR dy) { int ierr=0;
   return c_stroke_rounded_rect(fmkr, Get_FM(fmkr, &ierr), Number_to_double(x, &ierr), Number_to_double(y, &ierr), 
      Number_to_double(width, &ierr), Number_to_double(height, &ierr), Number_to_double(dx, &ierr), Number_to_double(dy, &ierr), &ierr); }
OBJ_PTR FM_fill_and_stroke_rounded_rect(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR width, OBJ_PTR height, OBJ_PTR dx, OBJ_PTR dy) { int ierr=0;
   return c_fill_and_stroke_rounded_rect(fmkr, Get_FM(fmkr, &ierr), Number_to_double(x, &ierr), Number_to_double(y, &ierr), 
      Number_to_double(width, &ierr), Number_to_double(height, &ierr), Number_to_double(dx, &ierr), Number_to_double(dy, &ierr), &ierr); }
OBJ_PTR FM_clip_circle(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR dx) { int ierr=0;
   return c_clip_circle(fmkr, Get_FM(fmkr, &ierr), 
      Number_to_double(x, &ierr), Number_to_double(y, &ierr), Number_to_double(dx, &ierr), &ierr); } 
OBJ_PTR FM_fill_circle(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR dx) { int ierr=0;
   return c_fill_circle(fmkr, Get_FM(fmkr, &ierr), 
      Number_to_double(x, &ierr), Number_to_double(y, &ierr), Number_to_double(dx, &ierr), &ierr); } 
OBJ_PTR FM_stroke_circle(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR dx) { int ierr=0;
   return c_stroke_circle(fmkr, Get_FM(fmkr, &ierr), 
      Number_to_double(x, &ierr), Number_to_double(y, &ierr), Number_to_double(dx, &ierr), &ierr); } 
OBJ_PTR FM_fill_and_stroke_circle(OBJ_PTR fmkr, OBJ_PTR x, OBJ_PTR y, OBJ_PTR dx) { int ierr=0;
   return c_fill_and_stroke_circle(fmkr, Get_FM(fmkr, &ierr), 
      Number_to_double(x, &ierr), Number_to_double(y, &ierr), Number_to_double(dx, &ierr), &ierr); } 
OBJ_PTR FM_append_frame_to_path(OBJ_PTR fmkr) { int ierr=0; return c_append_frame_to_path(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_fill_frame(OBJ_PTR fmkr) { int ierr=0; return c_fill_frame(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_stroke_frame(OBJ_PTR fmkr) { int ierr=0; return c_stroke_frame(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_fill_and_stroke_frame(OBJ_PTR fmkr) { int ierr=0; return c_fill_and_stroke_frame(fmkr, Get_FM(fmkr, &ierr), &ierr); }
OBJ_PTR FM_clip_to_frame(OBJ_PTR fmkr) { int ierr=0; return c_clip_to_frame(fmkr, Get_FM(fmkr, &ierr), &ierr); }
...
OBJ_PTR FM_private_show_marker(
   ...
   return c_private_show_marker(fmkr, Get_FM(fmkr, &ierr), Number_to_int(integer_args, &ierr), stroke_width,
      ...
      fill_color, stroke_color, &ierr); }

// texout.c
OBJ_PTR FM_rescale_text(OBJ_PTR fmkr, OBJ_PTR scaling_factor) { int ierr=0;
   return c_rescale_text(fmkr, Get_FM(fmkr, &ierr), Number_to_double(scaling_factor, &ierr), &ierr); }
OBJ_PTR FM_show_rotated_text(OBJ_PTR fmkr, OBJ_PTR text, OBJ_PTR frame_side, OBJ_PTR shift,
   ...
   return c_show_rotated_text(fmkr, Get_FM(fmkr, &ierr), String_Ptr(text, &ierr), Number_to_int(frame_side, &ierr), Number_to_double(shift, &ierr),
      Number_to_double(fraction, &ierr), Number_to_double(scale, &ierr), Number_to_double(angle, &ierr), 
      Number_to_int(justification, &ierr), Number_to_int(alignment, &ierr), &ierr); }
OBJ_PTR FM_show_rotated_label(OBJ_PTR fmkr, OBJ_PTR text,
   ...
   return c_show_rotated_label(fmkr, Get_FM(fmkr, &ierr), String_Ptr(text, &ierr), Number_to_double(xloc, &ierr), Number_to_double(yloc, &ierr),
